...
import com.bfc.putaway.databinding.FragmentTwoPutawayBinding
 ...
 * Use the [TwoPutawayFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class TwoPutawayFragment : Fragment() {
...
//    private var tv_tiHi: TextView? = null
//    private var edit_scan: EditText? = null

    private lateinit var bindind: FragmentTwoPutawayBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as MainActivityPutAway).setScreenTitle("RC200.02S RF Putaway")
        AppColors.setBackgroundColor(bindind.root, AppColors.screen_bc, activity)
        ...
        if (mMineUserEntity != null && mMineUserEntity.Output != null) {
            if (!mMineUserEntity.Output.w2item.isNullOrEmpty()) {
                bindind.tvDataAny?.text = mMineUserEntity.Output.w2item
            }

            if (!mMineUserEntity.Output.des01.isNullOrEmpty()) {
                bindind.tvToData?.text = mMineUserEntity.Output.des01
            }

            if (!mMineUserEntity.Output.des02.isNullOrEmpty()) {
                bindind.tvQtyData?.text = mMineUserEntity.Output.des02
            }

            if (!mMineUserEntity.Output.des03.isNullOrEmpty()) {
                bindind.tvTiHiData?.text = mMineUserEntity.Output.des03
            }

            if (!mMineUserEntity.Output.des04.isNullOrEmpty()) {
                //edit_scan?.setText(mMineUserEntity.Output.des04)
            }
            ...
            if (mMineUserEntity.Output.screen_bc != null) {
                AppColors.setBackgroundColor(bindind.root, mMineUserEntity.Output.screen_bc, activity)
            }
    ...
    override fun onCreateView(
        ...
        bindind = FragmentTwoPutawayBinding.inflate(inflater,container,false)
//        val rootView = inflater.inflate(R.layout.fragment_two_putaway, container, false)
//        tv_dataAny = rootView.findViewById<TextView>(R.id.tv_dataAny)
//        tv_To = rootView.findViewById<TextView>(R.id.tv_toData)
//        tv_qty = rootView.findViewById<TextView>(R.id.tv_qtyData)
//        tv_tiHi = rootView.findViewById<TextView>(R.id.tv_tiHiData)
//        edit_scan = rootView.findViewById<EditText>(R.id.edit_scan)
    ...
    companion object {
         ...
         * @return A new instance of fragment TwoPutawayFragment.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance(param1: String, param2: String) =
            TwoPutawayFragment().apply {
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        PutAwayBaseActivity.currentFragment = TwoPutawayFragment()
    }

    fun onNextClick(btnValue: String) {
        val mMap = ArrayMap<String, String>().apply {
            put("scanSlot", bindind.editScan?.text.toString())
            put("distance", SCANNED_DISTANCE)
            put("height", HEIGHT)
            put("Rfid_Tag", RFID)
        }
        listener?.onNextInteraction(2, btnValue, mMap)
    }
    ...
    fun fabClickedData() {
        ...
        listener?.onFabInteraction(2, listofButton,mMap)
    }

    fun updateUiOnScan(decodedData: String) {
        bindind.editScan?.setText(decodedData)
        ...
        if (!distance.isNullOrEmpty()) {
            ...
            if (mDistance > DISTANCE) {
                ...
                if (!Rfid_Tag.isNullOrEmpty() && !height.isNullOrEmpty()) {
                    ...
                    listener?.onNextInteraction(2, resources.getString(R.string.next), mMap)
                }
            }
        }
